...
class StorageRepair(private val context: Context) : RepairModule {
    ...
    fun cleanTempFiles(): Boolean { var cleaned=false; val d=File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).path); if(d.exists())d.listFiles()?.forEach{ if(it.name.endsWith(".apk")||it.name.endsWith(".tmp")){ it.delete(); cleaned=true } }; return cleaned }
}