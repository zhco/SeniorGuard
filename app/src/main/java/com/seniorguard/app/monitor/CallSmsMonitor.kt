...
class CallSmsMonitor(private val context: Context, private val engine: RepairEngine) {
    ...
    fun register() {
        try { context.registerReceiver(smsReceiver, IntentFilter("android.provider.Telephony.SMS_RECEIVED")) } catch(_:SecurityException){}
        try { context.registerReceiver(callReceiver, IntentFilter("android.intent.action.PHONE_STATE")) } catch(_:SecurityException){}
    }
    fun unregister() { try { context.unregisterReceiver(smsReceiver) } catch (_: Exception) {}; try { context.unregisterReceiver(callReceiver) } catch (_: Exception) {} }
        ...
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.getStringExtra(TelephonyManager.EXTRA_STATE) == TelephonyManager.EXTRA_STATE_RINGING) {
                if (isFraud(intent.getStringExtra(TelephonyManager.EXTRA_INCOMING_NUMBER)?:"")) {
                    try { (context.getSystemService(Context.TELECOM_SERVICE) as android.telecom.TelecomManager).endCall() } catch(_:Exception){}
                }
            }
        }
    }
